...
pub trait RenderTarget {
    ...
    ///
    /// 1. **Pixmap 跨帧持久**：不能每帧递一张新的。宿主的局部帧只写脏区，脏区之外直接
    ///    沿用里面已有的像素。
    /// 2. **缓冲重建的那一帧，平台必须先调 [`AppHandler::request_full_frame`]**，再把
    ///    目标交给 `render()`。新缓冲内容不完整，照常出局部帧的话，上屏后除脏区外全是
    ///    底色。参见 `platform/win32/mod.rs` 与 `platform/macos/window.rs` 的 `fresh` 分支。
    ///
    /// 宿主自己也会拦一道（`app/damage.rs` 里拿本帧物理尺寸与上一帧比），但那**只覆盖
    /// 尺寸变了**这一种重建。**尺寸不变的重建仍然只能靠第 2 条**——典型的是 macOS 的
    /// `degrade_to_software`：GPU 窗口降级到软路径，窗口一点没变，pixmap 却是头一回分配。
    ///
