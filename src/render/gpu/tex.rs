...
//! # 缓存键是图片身份，不是像素
//!
//! 键取 [`Image::cache_id()`]（构造时分配的单调递增 id）+ 像素尺寸，与 d2d 后端的
//! `image_cache` 同一条路子。图片是不可变资源（`Image` 内部 `Rc<Pixmap>`，克隆廉价且
//! 共享同一份像素），于是身份就够了，不需要对像素做哈希——一张 4K 图每帧哈希一遍比
//! 重新上传还贵。
//!
//! 这个 id 曾经是 `Rc<Pixmap>` 的**指针值**，因释放后地址复用会命中旧图的缓存（画出
//! 上一张图）而被换掉，来龙去脉记在 [`Image::cache_id`] 的文档里。尺寸留在键里只是
//! 廉价的一致性校验，不再承担防碰撞职责。
//!
//! # 淘汰策略：比 d2d 温和一档
//!
