...
3. **代码 key 存在性 lint**。**实到：不扫源码，改成一张手写清单**
   （`i18n::tests::builtin_covers_every_key_the_library_asks_for`）。原设计要从源码 grep 出
   `tr!("…")` 的字面 key 再校验——那等于让被测对象自己提供期望值，正是「测试自证循环」
   那条教训说的形态：删掉一处调用，测试跟着不再检查它，照样全绿。清单是手写的独立信源。

   前两项则做成了**库的公开 API** `i18n::lint::check(&[&str])`：下游应用会遇到一模一样的
   三种静默错（漏 key、占位符改名、变体表漏 `other`），没理由让每个项目各写一遍。
