...
> **定位红线**：始终是"轻量小工具"。两条约束——
> ① **tiny-skia CPU 软光栅恒为默认**；GPU/D2D 仅作**不替换默认的窗口级 opt-in**引入（v0.4.0 已落地，
>    见下"进展更新"），不强加于所有窗口、不引入 WGPU 字形图集（文字坚持 DirectWrite）；
...
> - **脏矩形局部重绘**：已交付——交互失效系统让 hover/拖动/点击/打字走 ~1ms 局部重绘（详见下"现状勘误"勘误说明）。
> - 另交付：渐变、暗色主题热切换、box-shadow + 子树 opacity、级联右键菜单、原生文件对话框（`PickDialog`）。
...
地基已交付（`src/i18n/`，设计见 `docs/i18n-design.md`）。下面是记下来的待办，按优先级排：

| 项 | 评级 | 优先级 | 说明 |
|----|------|--------|------|
| 内置更多语言的 `windui.*` 译文 | 🟡 | P2 | 现在只有 zh-CN / en 两份，用第三种语言的应用得自己补那 8 条（debug 下已 warn 提醒、`languages_missing_framework_strings` 可写进测试）。要不要由库来背 ja/ko/de/fr/ru… 的翻译质量，是个策略问题 |
| 托盘菜单 / 菜单栏文案跟随换语言 | ✅ 已交付 | — | `TrayMenuItem::item/check` 与 `MenuBarEntry::new` 收 `TextContent`：托盘菜单每次弹出才构建、菜单栏标题每帧现取，故都自动跟随。**托盘提示**（`set_tooltip`）仍需应用自己改 |
| 应用侧的 key 存在性 lint | ✅ 已交付 | — | `i18n::lint::check_usage(dir, &locales)` 扫 `.rs` 里的字面 key，报文件名+行号；扫描器跳过注释与字符串。只查"用了没翻译"一个方向（反向因动态 key 必然误报）。库自己另有一条同源测试盯 `windui.*` 写错字 |
| 数字 / 日期 / 货币本地化格式 | 🟡 | P2 | 要么引 `icu_*`（重），要么手写常用档。先等真实需求 |
| 小数复数（`1.0 files`） | 🔴 | P3 | `count` 目前只看整数。CLDR 的 `v`/`w`/`f` 操作数没实现 |
| **译文文件监听（改完自动生效）** | 🔴 | **P3（低）** | 现有 `LocaleHandle::reload()` 手动刷新对开发期已经够用；自动监听要拉一个 watcher 依赖 + 一条后台线程，换来的只是省一次点击 |
| RTL / BiDi 排版 | 🔴 | — | 需要文字栈先支持双向重排（多行是自绘视觉行布局、按字符切分），属另一条线，不是 i18n 层能补的洞 |

## 现状勘误（重要）

> **已解决（v0.4.0）**：下述"全量重绘、无脏矩形"的现状已被交付的**交互失效系统**改变——
> hover/拖动/点击/打字经结构签名判定走 ~1ms 局部重绘，仅结构性变更才升整窗。以下为历史背景，留作设计脉络。

外部分析称框架"仅脏标记时才布局、Clip Rect 脏区裁剪避免全屏绘制"——**与（当时）现状不符**。
...
- **已交付（v0.4.0）**：Signals（`Signal<T>` 状态原语）、脏矩形局部重绘、GPU/D2D opt-in 后端。
- **仍不做**：富文本 / 行内混排（小工具少需、成本高）。
