...
| 托盘菜单 / 菜单栏文案跟随换语言 | 🟡 | P2 | 现在走 `tr!` 当场定格，重建时才更新。与右键菜单同理（每次弹出重建）的部分已经够用，常驻托盘菜单是例外 |
| 应用侧的 key 存在性 lint | 🟡 | P2 | 库自己那批靠一张手写清单钉住（故意不扫源码，见 `docs/i18n-design.md` §7）；应用侧目前没有工具扫 `t!("…")` 的字面 key |
