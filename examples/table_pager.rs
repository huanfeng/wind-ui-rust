...
//! - 勾上「只看 .log」→ 条目变少 → **当前页若越界会自动钳到末页并重取**。少了这一步，
//!   界面会停在"第 21 / 5 页"配一张空表，而且怎么点都回不来。
...
fn main() {
    ...
    let toolbar = Element::row()
        ...
        .spacing(12)
        .child(
            Element::checkbox("只看 .log（条目变少，越界的页码会自动钳回来）", only_log).on_click(
                move |_ctx| {
                    // 换筛选条件回到第一页：留在第 20 页看另一批数据没有意义。
                    page.set(0);
                    reload(rows, total, page, only_log, sort);
                },
            ),
