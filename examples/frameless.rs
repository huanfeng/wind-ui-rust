...
fn main() {
    ...
    let title_bar = Element::row()
        ...
        .window_drag()
        .child(Element::label("   windui — 无边框窗口").font_size(14.0).fg(Color::WHITE).weight(1.0))
    ...
    let body = Element::col()
        ...
        .padding(24)
        .spacing(10)
        .child(Element::label("自定义标题栏").font_size(20.0).fg(Color::hex(FG)).height(28).width_match())
        .child(
            Element::label("拖动深色标题栏移动窗口；右上角按钮最小化/最大化/关闭；窗口边缘可缩放。")
                .font_size(13.0)
                .fg(Color::hex(SUB))
                .width_match()
                .weight(1.0),
    ...
    let ui = Element::col().fill().child(title_bar).child(body.weight(1.0));
