...
核心指标是**极低内存**与**空闲零 CPU**——任何改动都不应破坏这两点。

内存要**连同窗口尺寸与 DPI 一起说**才有意义：大头是软件光栅按**物理**像素留的约 2.5 份
全屏 RGBA 缓冲，200% 缩放下同一窗口就是 100% 的 4 倍。当前基准（`scripts/measure_footprint.ps1`
实测）：最小窗口 480×320 私有 2.7MB@100% / 4.6MB@200%；关于窗 620×556 私有 5.5MB@100% /
14.2MB@200%。判断"是不是回归"请在同一 DPI 下跑同一个示例比对，别拿裸数字对号入座。
...
- **截图验证**（离屏渲染，`App::screenshot_from_args()`）：
  ...
  - `--click X Y` / `--rclick X Y`：截图前合成左/右键（验证下拉展开、标签切换、列表选中、右键菜单等）。
    坐标是**空格分隔的两个参数**，不是 `X,Y`——写成逗号会被静默忽略（解析不出整数就不合成），
    表现为"截图跟没点一样"。
  - `--drag X0 Y0 X1 Y1`：截图前合成一次左键拖拽（Down→Move→Up），验证划选高亮、拖动排序
    这类**只有拖出去才成立**的视觉——单击到不了这些状态。Up 之后额外出一帧，因此也覆盖
    "选区能否活过宿主对 Up 触发的重排"。
  - `--type <text>` / `--key <name>`：截图前合成键盘输入。两者与 `--click` 一样可重复，且
    **按写的顺序混合回放**（`--type ab --key Enter --type c` 严格按此序）。走的是
    `handler.on_key`，与真实按键同一条通路——焦点裁决、Tab 兜底、控件回调都照常。
    没有它时，任何"输入中"的界面状态都截不到图（补全浮层、候选高亮、焦点落在哪），
    键盘通路是唯一没有视觉回归覆盖的核心交互。
    `--key` 认具名键 `Enter/Escape/Tab/Up/Down/Left/Right/Home/End/Backspace/Delete/Space`
    与单个字符，可加 `ctrl+` / `shift+` 前缀（`--key ctrl+a` 全选）。认不出会打一行提示并
    跳过——不猜，因为猜错的症状是"截图跟没按一样"，无声无息。
    `--type` 逐字符发 `Key::Char`，不经平台键码映射，故中文也能直接打。
  - **命中诊断**：合成点击**落空时总会打印一行**（未命中任何节点 / 命中了但无人消费），
    因为落空是完全无声的——不报错不 panic，截出来的图和没点一样，于是"坐标写错了"与
    "框架丢了这次点击"在现象上分不开。`WINDUI_HITS=1` 额外打印成功命中的节点与矩形，
    用于确认"内容重建改变布局后，旧坐标现在指向谁"。
  - `--size W H`：覆盖 `App::new` 的窗口尺寸，并把 `min_size` 的下限**一并放开**——要测的
    正是下限处的表现，还受钳制就永远截不到那张图。没有它时尺寸写死在代码里，"最小尺寸下
    布局是否还成立"只能改代码重编译，进不了 CI。
  - 合成交互期间 `ctx.open_window` 开出的子窗**各自再出一张**：`out.png` → `out-1.png`、
