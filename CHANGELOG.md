...
- **虚拟滚动列表与表格**：`Element::virtual_list(data, row_height, row_fn)` 与
  ...
  已知限制（见 `docs/API_GUIDE.md` §6.6 的限制表）：行高必须固定（表格多行单元格因此
  不适用）、Tab 焦点环只覆盖已渲染的行、行的内部临时状态随重建重置、与拖拽重排互斥、
  不做内部排序。**几百行以内用 `list_signal` 就好**——它没有上面任何一条限制。
  新增 `examples/virtual_list.rs`；`fullshowcase` 的控件页与表格页各加一张卡片。
