...
- **`App::title` 收 `TextContent`**，窗口标题因此能跟随换语言，也能绑 `Signal<String>`。
  标题不经 paint、是一次性交给系统的字符串，走**拉取式**：平台在事件分发后与出帧后各问
  一次，宿主现算并与上次推送的比对，变了才发。没写 `App::title` 的应用一次多余的系统调用
  都不会发生。子窗口（`Window::new`）的标题暂不跟随。
